...
local function lockpickFinish(success)
	if success then
		...
		Wait(1800)
		TriggerServerEvent('sjrp-doorlock:server:updateState', closestDoor.id, false, false, true, false) -- Broadcast new state of the door to everyone
	else
...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		QBCore.Functions.TriggerCallback('sjrp-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	QBCore.Functions.TriggerCallback('sjrp-doorlock:server:setupDoors', function(result)
...
RegisterNetEvent('sjrp-doorlock:client:setState', function(serverId, doorID, state, src, enableAnimation, enableSounds)
...
RegisterNetEvent('sjrp-doorlock:client:addNewDoor', function()
	canContinue = false
	hideNUI()
	local doorData = {}
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		TriggerServerEvent('sjrp-doorlock:server:saveNewDoor', doorData, false)
		canContinue = true
	else
		...
		doorData.model = model
		doorData.heading = heading
		TriggerServerEvent('sjrp-doorlock:server:saveNewDoor', doorData, true)
		canContinue = true
	end
end)

RegisterNetEvent('sjrp-doorlock:client:newDoorAdded', function(data, id)
	Config.DoorList[id] = data
	TriggerEvent('sjrp-doorlock:client:setState', PlayerData.source, id, data.locked, false, true, true)
end)
...
RegisterCommand('toggledoorlock', function()
	...
	TriggerServerEvent('sjrp-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
end)
