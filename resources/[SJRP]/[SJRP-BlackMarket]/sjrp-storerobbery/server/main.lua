...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function(safe)
    ...
    if luck <= 50 then
        Player.Functions.AddItem("red_usb", math.random(1, 2))
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["red_usb"], "add")
        if luck == odd then
            Wait(500)
            Player.Functions.AddItem("red_usb", 1)
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["red_usb"], "add")
        end
    end
end)
