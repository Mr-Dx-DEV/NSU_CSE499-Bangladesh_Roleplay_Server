            ...
            for _, loc in pairs(position) do
                ...
                if dist <= 500.0 then
                    if not DoesEntityExist(dealer) then
                        ...
                        dealer = CreatePed(26, 'a_m_y_business_03', loc["x"], loc["y"], loc["z"] - 1 , loc["h"], false, false)
...
Citizen.CreateThread(function()
exports['sjrp-target']:AddTargetModel(`a_m_y_business_03`, {
	options = {
		{
			event = "startRobbery",
...
AddEventHandler("getRandomHouseLoc", function()
    local missionTarget = Config.Location[math.random(#Config.Location)]
