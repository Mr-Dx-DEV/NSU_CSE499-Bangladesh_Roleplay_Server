...
function ExchangeSuccess()
    ...
    -- open ndoor
    TriggerServerEvent('sjrp-doorlock:server:updateState', 'Jewellery', false, false, false, true, false)

    -- close door after 1.5 minutes
    Wait(90000)
    QBCore.Functions.Notify('Cops are on the way! Doors are now locked.', 'error')
    TriggerServerEvent('sjrp-doorlock:server:updateState', 'Jewellery', true, false, false, true, false)
end
...
function callPolice(missionTarget)
    local data = {displayCode = '459', description = 'Jewellery Robbery In Progress', isImportant = 0, recipientList = {'police'}, length = '10000', infoM = 'fa-info-circle', info = 'Jewellery alarm activated'}
