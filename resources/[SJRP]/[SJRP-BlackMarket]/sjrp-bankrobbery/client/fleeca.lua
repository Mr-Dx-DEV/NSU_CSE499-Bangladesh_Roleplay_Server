...
local function OpenPaletoDoor()
    TriggerServerEvent('sjrp-doorlock:server:updateState', 85, false)
                                ...
                                QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                    if result then
                                        ...
                                        QBCore.Functions.Progressbar("hack_gate", "Connecting the hacking device ..", math.random(5000, 10000), false, true, {
                                            ...
                                            TriggerServerEvent("QBCore:Server:RemoveItem", "red_usb", 1)
                                            TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["red_usb"], "remove")
...
CreateThread(function()
    ...
    local requiredItems = {
        ...
        [2] = {name = QBCore.Shared.Items["red_usb"]["name"], image = QBCore.Shared.Items["red_usb"]["image"]},
    }
    while true do
        local ped = PlayerPedId()
        local pos = GetEntityCoords(ped)
