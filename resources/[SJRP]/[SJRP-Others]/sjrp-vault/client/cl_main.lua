...
CreateThread(function()
    for k, v in pairs(Config.location) do
        exports['sjrp-target']:AddBoxZone(v.name, v.coords, v.length, v.width, {
            name = v.name,
            heading = v.heading,
            debugPoly = v.debug,
            minZ = v.minz,
...
AddEventHandler('re2-vault:viewStorages', function(location)
    ...
    if result then
        ...
        exports['sjrp-menu']:openMenu(storagesMenu)
    else
    ...
    if dialog ~= nil then
        ...
        if result then
            ...
            table.insert(storageMenu,closeMenu) 
              exports['sjrp-menu']:openMenu(storageMenu)
...
AddEventHandler('re2-vault:createStorage', function(location)
    local player=QBCore.Functions.GetPlayerData()
    local citizenid=player.citizenid
    local cpdialog = exports['sjrp-input']:ShowInput({
...
AddEventHandler('re2-vault:addMemberToStorage', function(data)
    ...
    local citizenid=player.citizenid
    -- local storagename=location.."_"..citizenid
    local mdialog = exports['sjrp-input']:ShowInput({
        header = "Add Member",
        submitText = "Submit",
        inputs = {
            {
