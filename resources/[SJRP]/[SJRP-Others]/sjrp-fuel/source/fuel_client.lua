...
exports['sjrp-target']:AddTargetModel(props, {
	options = {
		{
			type = "client",
			event = "sjrp-fuel:client:buyCanMenu",
			icon = "fas fa-burn",
			label = "Buy Jerry Can",
		},
		{
			type = "client",
			event = "sjrp-fuel:client:refuelCanMenu",
...
RegisterNetEvent('sjrp-fuel:client:buyCan', function()
    if not HasPedGotWeapon(ped, 883325847) then
		if QBCore.Functions.GetPlayerData().money['cash'] >= Config.canCost then
        	...
        	TriggerServerEvent('sjrp-fuel:server:PayForFuel', Config.canCost, GetPlayerServerId(PlayerId()))
		else
...
RegisterNetEvent('sjrp-fuel:client:refuelCanMenu', function()
	exports['sjrp-menu']:openMenu({
		{
			header = "Gas Station",
			txt = "Buy jerry can. Remember there will be a 10% tax fee.",
			params = {
				event = "sjrp-fuel:client:refuelCan",
			}
		},
	})
...
RegisterNetEvent('sjrp-fuel:client:refuelCan', function()
	...
	if GetAmmoInPedWeapon(ped, 883325847) < 4500 then
		...
		while not HasAnimDictLoaded('weapon@w_sp_jerrycan') do Citizen.Wait(100) end
			...
			TriggerServerEvent('sjrp-fuel:server:PayForFuel', Config.refuelCost, GetPlayerServerId(PlayerId()))
			SetPedAmmo(ped, 883325847, 4500)
...
RegisterNetEvent('sjrp-fuel:client:SendMenuToServer', function()
	local vehicle = QBCore.Functions.GetClosestVehicle()
	local CurFuel = GetVehicleFuelLevel(vehicle)
	local refillCost = Round(Config.RefillCost - CurFuel) * Config.CostMultiplier

	if CurFuel < 95 then
		TriggerServerEvent('sjrp-fuel:server:OpenMenu', refillCost, inGasStation)
	else
...
-- refuel vehicle 
RegisterNetEvent('sjrp-fuel:client:RefuelVehicle', function(refillCost)
	local vehicle = QBCore.Functions.GetClosestVehicle()
	local ped = PlayerPedId()
	local CurFuel = GetVehicleFuelLevel(vehicle)
	...
	if inGasStation then
		if isCloseVeh() then
			...
			QBCore.Functions.Progressbar("refuel-car", "Refueling", time, false, true, {
				...
				TriggerServerEvent('sjrp-fuel:server:PayForFuel', refillCost, GetPlayerServerId(PlayerId()))
				SetFuel(vehicle, 100)
				PlaySound(-1, "5_SEC_WARNING", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
				StopAnimTask(ped, "weapon@w_sp_jerrycan", "fire", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
			end, function() -- Cancel
