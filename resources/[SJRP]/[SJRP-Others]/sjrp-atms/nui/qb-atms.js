...
function closeBanking() {
    $.post("https://sjrp-atms/NUIFocusOff", JSON.stringify({}));
    currentLimit = null;
    clientPin = null;
    cardSelected = null;
    clientCid = null;
...
$( function() {
        ...
        if ($('#pinContainer').is(':visible')) {
            if (key.which >= 48 && key.which <= 57) { // 48 - 57 (0-9 keys)
                ...
                if (currentVal == clientPin) {
                    ...
                    $('#pinCode').val('');
                    $.post("https://sjrp-atms/loadBankingAccount", JSON.stringify({
                        cid: clientCid,
                        cardnumber: cardNumb
                    }));
                } else {
    ...
    $(document).on('click','[data-action=useCard]',function(){
        $.post("https://sjrp-atms/playATMAnim", JSON.stringify({}));
    ...
    $(document).on('click','[data-action=removeCard]',function(){
        var selectedCard = $(this).data('card')
        $.post("https://sjrp-atms/removeCard", JSON.stringify({
    ...
    $(document).on('click','[data-action=ATMwithdraw]',function(){
        ...
        if(amount !== undefined && amount !== null && amount !== 0) {
            $.post("https://sjrp-atms/doATMWithdraw", JSON.stringify({
