...
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst) -- Triggered in sjfw
    hunger = newHunger
    thirst = newThirst
end)
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        lastFuelCheck = math.floor(exports['sjrp-fuel']:GetFuel(vehicle))
    end
    ...
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) then
                if exports['sjrp-fuel']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
