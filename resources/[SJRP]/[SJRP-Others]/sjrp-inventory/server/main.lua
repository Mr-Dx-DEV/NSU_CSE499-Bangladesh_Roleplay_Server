...
RegisterNetEvent("inventory:server:GiveItem", function(target, inventory, item, amount)
	...
	if amount <= item.amount then
		...
		if OtherPlayer.Functions.AddItem(item.name, amount, false, item.info) then
			...
			TriggerClientEvent('crfw-inventory:client:giveAnim', src)
			TriggerClientEvent('crfw-inventory:client:giveAnim', target)
		else
...
RegisterNetEvent('crfw-inventory:server:SaveStashItems', function(stashId, items)
    exports.oxmysql:insert('INSERT INTO stashitems (stash, items) VALUES (:stash, :items) ON DUPLICATE KEY UPDATE items = :items', {
        ['stash'] = stashId,
        ['items'] = json.encode(items)
    })
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				local toItemData = exports['sjrp-traphouse']:GetInventoryData(traphouseId, toSlot)
				local IsItemValid = exports['sjrp-traphouse']:CanItemBeSaled(fromItemData.name:lower())
				if IsItemValid then
					Player.Functions.RemoveItem(fromItemData.name, fromAmount, fromSlot)
					TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
					if toItemData ~= nil then
						...
						local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
						if toItemData.name ~= fromItemData.name then
							exports['sjrp-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount)
							Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
					...
					local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
					exports['sjrp-traphouse']:AddHouseItem(traphouseId, toSlot, itemInfo["name"], fromAmount, fromItemData.info, src)
				else
				...
				if toInventory == nil or toInventory == 0 then
					...
					if itemInfo["name"] == "radio" then
						TriggerClientEvent('sjrp-radio:onRadioDrop', src)
					end
				end
			end
		else
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "traphouse" then
		...
		local fromItemData = exports['sjrp-traphouse']:GetInventoryData(traphouseId, fromSlot)
		local fromAmount = tonumber(fromAmount) ~= nil and tonumber(fromAmount) or fromItemData.amount
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			if toInventory == "player" or toInventory == "hotbar" then
				local toItemData = Player.Functions.GetItemBySlot(toSlot)
				exports['sjrp-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], fromAmount)
				if toItemData ~= nil then
					local itemInfo = QBCore.Shared.Items[toItemData.name:lower()]
					local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						Player.Functions.RemoveItem(toItemData.name, toAmount, toSlot)
						exports['sjrp-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
					else
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						local itemInfo = QBCore.Shared.Items[toItemData.name:lower()]
						exports['sjrp-traphouse']:RemoveHouseItem(traphouseId, toSlot, itemInfo["name"], toAmount)
						exports['sjrp-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
					end
				end
				local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
...
QBCore.Functions.CreateCallback('crfw-inventory:server:GetStashItems', function(source, cb, stashId)
		...
		if amount > 0 then
			if itemData ~= nil then
				...
				elseif itemData["name"] == "harness" then
					info.uses = 20
				elseif itemData["name"] == "cigar" then
                    info.uses = 20
