...
const OpenMenu = (data) => {
    ...
    $("#sjrp-input-form").on("change", function (event) {
        formInputs[$(event.target).attr("name")] = $(event.target).val();
    });

    $("#sjrp-input-form").on("submit", async function (event) {
        if (event != null) {
            event.preventDefault();
        }
        let formData = $("#sjrp-input-form").serializeArray();
