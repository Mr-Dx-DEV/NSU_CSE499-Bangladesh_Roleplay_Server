...
--[[ Main Event Handlers ]]--
    ...
    RegisterNetEvent("K9:UpdateLanguage", function(commands)
        language = commands
        --Trace(tostring(json.encode(language)))
    end)

    -- Opens K9 Menu
    RegisterNetEvent("K9:OpenMenu", function(pedRestriction, pedList)
        if pedRestriction then
            if CheckPedRestriction(GetLocalPed(), pedList) then
                ...
                QBCore.Functions.Notify("You do not have the right PED to use the K9.", "error")
            end
        else
            EnableMenu()
        end
    ...
    RegisterNetEvent("K9:IdentifierRestricted", function()
        ---Notification(tostring("~r~You do not match any identifiers in the whitelist."))
            QBCore.Functions.Notify("You do not match any identifiers in the whitelist.", "error")
    end)

    -- Spawns and Deletes K9
    RegisterNetEvent("K9:ToggleK9", function(model)
        if spawned_ped == nil then
    ...
    RegisterNetEvent("K9:ToggleFollow", function()
        if spawned_ped ~= nil then
            if not following then
                ...
                if has_control then
                    ...
                    QBCore.Functions.Notify("Your pet is following you", "error")
                end
            else
                local has_control = false
                RequestNetworkControl(function(cb)
                ...
                if has_control then
                    ...
                    QBCore.Functions.Notify("Your pet is not following you", "error")
                end
            end
        end
    end)
    ...
    RegisterNetEvent("K9:ToggleVehicle", function(isRestricted, vehList)
        if not searching then
            if IsPedInAnyVehicle(spawned_ped, false) then
                ...
                QBCore.Functions.Notify("Your pet exited the vehicle", "error")
                Wait(2000)
                    ...
                    if door ~= false then
                        if isRestricted then
                            if CheckVehicleRestriction(vehicle, vehList) then
                                ...
                                QBCore.Functions.Notify("Your pet entered the vehicle", "error")
                            end
                        else
                            TaskEnterVehicle(spawned_ped, vehicle, -1, door, 2.0, 1, 0)
                            --Notification(tostring(k9_name .. " " .. language.enter))
    ...
    RegisterNetEvent("K9:ToggleAttack", function(target)
        if not attacking and not searching then
            ...
            QBCore.Functions.Notify("You commanded your pet to attarck some one", "error")
        end
    end)

    -- Triggers K9 to Search Vehicle
    RegisterNetEvent("K9:SearchVehicle", function(items, openDoors)
        local vehicle = GetVehicleAheadOfPlayer()
        Trace(tostring(vehicle))
        Trace(tostring(json.encode(items)))
        if vehicle ~= 0 and not searching then
            searching = true
            local found_table = {}

            ---Notification(tostring(k9_name .. " has began searching..."))
            QBCore.Functions.Notify("has began searching...", "error")
...
--[[ Threads ]]
    ...
    CreateThread(function()
        while true do
            ...
            -- Trigger Opens Menu F2
            --[[ if IsControlPressed(1, 167) then
                TriggerServerEvent("K9:RequestOpenMenu")
            end ]]

            -- Trigger Follow F5
            if IsControlJustPressed(1, 166) and not IsPlayerFreeAiming(PlayerId()) then
    ...
    RegisterKeyMapping('petmenu', 'PetMenu', 'keyboard', 'F7')
