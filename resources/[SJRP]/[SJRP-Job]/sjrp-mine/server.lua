...
QBCore.Functions.CreateCallback('qb-mine:get', function(source, cb, item, craftable)
	local hasitem = false
...
RegisterServerEvent('qb-mine:GetItem', function(ItemMake, craftable)
	local src = source
	...
	--if you are making 4 items copper, iron or steel then you are smelting
	--the rest would be cutting, which would result in the ability to break drilbits
	if ItemMake == "copper" or ItemMake == "iron" or ItemMake == "steel" then
	else
...
RegisterServerEvent('qb-mine:MineReward', function()
...
RegisterServerEvent('qb-mine:CrackReward', function()
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterNetEvent("qb-mine:Selling", function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent("qb-mine:SellJewel", function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local currentitem = data
    if Player.Functions.GetItemByName(data) ~= nil then
...
QBCore.Functions.CreateCallback('qb-mine:Cutting:Check:Tools', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
