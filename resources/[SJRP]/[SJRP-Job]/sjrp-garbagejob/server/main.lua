...
QBCore.Functions.CreateCallback("garbagejob:server:NextStop", function(source, cb, currentStop, currentStopNum, currLocation)
    ...
    if(X >= 10) and (X <= 30) then
        Player.Functions.AddItem("metaldetector", 1, true)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["metaldetector"], 'add')
        TriggerClientEvent('QBCore:Notify', source, "You found an item")
    end
    if(X >= 31) and (X <= 40) then
        Player.Functions.AddItem("hunting_knife", 1, true)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["hunting_knife"], 'add')
        TriggerClientEvent('QBCore:Notify', source, "You found an item")
    end
    if(X >= 41) and (X <= 100) then
        Player.Functions.AddItem("drillbit", 1, true)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["drillbit"], 'add')
        TriggerClientEvent('QBCore:Notify', source, "You found an item")
    end
    if(X >= 101) and (X <= 150) then
        Player.Functions.AddItem("gameboy", 1, true)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["gameboy"], 'add')
        TriggerClientEvent('QBCore:Notify', source, "You found an item")
    end
    if(X >= 151) and (X <= 200) then
        Player.Functions.AddItem("goldcoin", 1, true)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["goldcoin"], 'add')
        TriggerClientEvent('QBCore:Notify', source, "You found an item")
    end
    if(X >= 201) and (X <= 210) then
        Player.Functions.AddItem("handcuffs", 1, true)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["handcuffs"], 'add')
        TriggerClientEvent('QBCore:Notify', source, "You found an item")
    end
    if(X >= 211) and (X <= 220) then
        Player.Functions.AddItem("greenphone", 1, true)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["greenphone"], 'add')
        TriggerClientEvent('QBCore:Notify', source, "You found an item")
    end
