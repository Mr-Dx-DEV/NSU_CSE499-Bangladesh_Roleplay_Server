...
QBCore.Functions.CreateCallback('GG_casinoheist:server:getCops', function(source, cb)
	local cops = 0
...
RegisterNetEvent('GG_casinoheist:server:spawnvault', function(type)
...
RegisterNetEvent('GG_casinoheist:server:recieveLockerItem', function(type)
    local src = source
    local ply = QBCore.Functions.GetPlayer(src)
    ply.Functions.AddItem('goldbar', math.random(1,5))
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['goldbar'], "add")
    chance = math.random(1, 10)
    if chance == 2 then
        ply.Functions.AddItem('goldbar', math.random(1,3))
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['goldbar'], "add")
    end
end)

RegisterNetEvent('GG_casinoheist:server:recieveCartItem', function(type)
    local src = source
    local ply = QBCore.Functions.GetPlayer(src)
    local info = {
...
RegisterNetEvent('GG_casinoheist:server:recieveGoldItem', function(type)
    local src = source
    local ply = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('GG_casinoheist:server:callCops', function(streetLabel, coords)
...
RegisterNetEvent('aj:sync', function(status)
    ...
    elseif status == false then
        ...
        TriggerClientEvent('GG_casinoheist:client:ExplodeAnim' , -1)
