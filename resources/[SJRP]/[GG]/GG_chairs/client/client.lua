...
CreateThread(function()
	for k, v in pairs(Config.Locations) do
		...
		--Add sjrp-target Locations if enabled in config
		if Config.Target then
			exports['sjrp-target']:AddCircleZone("ChairStore"..k, v.location, 2.0, { name="ChairStore"..k, debugPoly=false, useZ=true, }, 
			{ options = { { event = "GG_chairs:openShop", icon = "fas fa-chair", label = "Browse Store", store = v.store }, },
				distance = 2.0
			})
		end	
	end
...
--Chair Store Opening
RegisterNetEvent('GG_chairs:openShop')
AddEventHandler('GG_chairs:openShop', function(data)
...
RegisterNetEvent("GG_chairs:Use")
AddEventHandler("GG_chairs:Use", function(item)
