...
local GunDrop = BoxZone:Create(vector3(1064.3, 2363.18, 43.94), 42.8, 70.4, {
    name="Droploc",
    debugPoly= false,
    offset={1.0,1.0,1.0},
    scale={1.0,1.0,1.0},
...
GunDrop:onPlayerInOut(function(isPointInside)
    if isPointInside then
        Droploc = true
        PlaySoundFrontend(-1, "ScreenFlash", "MissionFailedSounds")
        QBCore.Functions.Notify("You entered The Red Alert Zone, Good luck!", "success", 5000)
    else
        Droploc = false
        PlaySoundFrontend(-1, "ScreenFlash", "MissionFailedSounds")
        QBCore.Functions.Notify("You exited The Red Alert Zone", "error", 5000)
    end
end)


-- Create the AirDrop
RegisterNetEvent("GG_gundrop:client:CreateDrop", function(weapon, ammo, roofCheck, planeSpawnDistance)
    local playerCoords = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 10.0, 0.0)    
    if Droploc == true then         
        QBCore.Functions.TriggerCallback("GG_gundrop:server:getCops", function(CurrentCops)
            if CurrentCops >= RequiredCops then

                TriggerEvent("GG_gundrop:client:StartDrop", weapon, tonumber(ammo), roofCheck or false, planeSpawnDistance or 400.0, {["x"] = playerCoords.x, ["y"] = playerCoords.y, ["z"] = playerCoords.z})    
                ...
                if weapon == "WEAPON_CARBINERIFLE" or weapon == "WEAPON_ADVANCEDRIFLE" then
                    TriggerServerEvent("GG_gundrop:server:RemoveItem", "goldenphone", 1)
                elseif weapon == "WEAPON_ASSAULTRIFLE" then
                    TriggerServerEvent("GG_gundrop:server:RemoveItem", "redphone", 1)
                elseif weapon == "WEAPON_ASSAULTSMG" then
                    TriggerServerEvent("GG_gundrop:server:RemoveItem", "greenphone", 1)
                end
        ...
        QBCore.Functions.Notify("Redwood Light")
end)

-- Start the AirDrop
RegisterNetEvent("GG_gundrop:client:StartDrop", function(weapon, ammo, roofCheck, planeSpawnDistance, dropCoords)
...
function CrateDrop(weapon, ammo, planeSpawnDistance, dropCoords)
    CreateThread(function()
        ...
        SetTimeout(1 * 60 * 1000, function()
            ...
            TriggerServerEvent("GG_gundrop:server:AddItem", weapon, 1)            
        end)
        end)
        end)  
    end)
...
RegisterNetEvent('GG_GunDrop_Alart', function(text)
    ...
    TriggerServerEvent('sjrp-phone:server:sendNewMail', {
        sender = 'Russian Mafia Alert',
        subject = 'Create Drop Alert',
        message = text,
        button = {}
    })
end)