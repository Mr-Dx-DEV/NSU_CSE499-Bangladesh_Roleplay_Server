$(function() {
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) { // Escape key
            $.post('https://sjrp-radio/escape', JSON.stringify({}));
            QBRadio.SlideDown()
        } else if (data.which == 13) { // Enter key
            $.post('https://sjrp-radio/joinRadio', JSON.stringify({
                channel: $("#channel").val()
            }));
        }
    };
...
$(document).on('click', '#disconnect', function(e){
    ...
    $.post('https://sjrp-radio/leaveRadio');
});

$(document).on('click', '#volumeUp', function(e){
    e.preventDefault();

    $.post('https://sjrp-radio/volumeUp', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#volumeDown', function(e){
    ...
    $.post('https://sjrp-radio/volumeDown', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#decreaseradiochannel', function(e){
    ...
    $.post('https://sjrp-radio/decreaseradiochannel', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#increaseradiochannel', function(e){
    ...
    $.post('https://sjrp-radio/increaseradiochannel', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#poweredOff', function(e){
    ...
    $.post('https://sjrp-radio/poweredOff', JSON.stringify({
        channel: $("#channel").val()
    }));
});
